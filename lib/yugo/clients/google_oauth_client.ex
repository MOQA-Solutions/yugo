defmodule Yugo.Clients.GoogleOAuthClient do 
  ...
  def handle_info({:timeout , timer , :"expired token"}, conn) when 
    conn.timer == timer, do: 
      exit("expired token") 
  ...
  defp on_authenticate_response(conn, :ok, _text) do
    timer = :erlang.start_timer(@expiration_timeout , self() , :"expired token")
    conn
  ...
  defp xoauth_core(username, token), do: 
    "user=#{username}" 
    <> 
    @soh
    <>
